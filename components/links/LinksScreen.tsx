  ...
  return (
    <div className="mx-auto max-w-lg space-y-5 px-4 pt-8 pb-8">
      ...
      <button
        ...
        className="w-full rounded-2xl bg-teal-600 py-3 font-semibold text-white shadow-sm"
      >
        + {s.add}
      </button>
        ...
        <div className="space-y-5">
          {grouped.map((g) => (
            <section key={g.country}>
              <h2 className="mb-2 text-sm font-bold text-teal-700">🌍 {g.country}</h2>
              <div className="space-y-3">
                {g.areas.map((a) => (
                  <div key={a.area || "_"}>
                    {a.area && (
                      <h3 className="mb-1 pr-1 text-xs font-semibold text-slate-400">
                        {a.area}
                      </h3>
                    )}
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            <a
                              ...
                              className="min-w-0 flex-1 font-semibold text-slate-800"
                            >
                              🔗 {l.title}
                            </a>
                          ...
                          <p className="mt-1 truncate text-xs text-slate-300" dir="ltr">
                            {l.url}
                          </p>
                        </li>
                      ))}
    ...
    <Sheet open onClose={onClose} title={link ? s.edit : s.add}>
      ...
      >
        <label className="block">
          <span className="mb-1 block text-sm font-medium text-slate-600">{s.linkTitle}</span>
          <input className={field} value={title} onChange={(e) => setTitle(e.target.value)} autoFocus />
        </label>
        <label className="block">
          <span className="mb-1 block text-sm font-medium text-slate-600">{s.url}</span>
          <input className={field} value={url} onChange={(e) => setUrl(e.target.value)} dir="ltr" inputMode="url" placeholder="https://…" />
        </label>
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="mb-1 block text-sm font-medium text-slate-600">{s.country}</span>
            <input className={field} value={country} onChange={(e) => setCountry(e.target.value)} list="link-countries" />
          </label>
          <label className="block">
            <span className="mb-1 block text-sm font-medium text-slate-600">{s.area}</span>
        ...
        <label className="block">
          <span className="mb-1 block text-sm font-medium text-slate-600">{s.note}</span>
          <textarea className={field} value={note} onChange={(e) => setNote(e.target.value)} rows={2} />
        </label>
        <button type="submit" className="w-full rounded-xl bg-teal-600 py-3 font-semibold text-white">
          {s.save}
        </button>
      </form>
    </Sheet>
